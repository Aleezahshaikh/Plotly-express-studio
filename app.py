...
fig7 = px.sunburst(sun_df, path=["income", "education", "Gender"], color="income")
...
# SCENARIO 10 — Treemap
# ========================================================================
scenario(10, "Population Breakdown: Race → Workclass → Income", "Treemap showing relative group sizes across race, workclass, and income.")
tree_df = filtered.dropna(subset=["workclass"])
fig10 = px.treemap(tree_df, path=["race", "workclass", "income"], color="race")
...
# ========================================================================
# SCENARIO 12 (bonus) — Parallel categories
# ========================================================================
scenario(12, "Parallel Categories: Workclass, Education Level & Income", "Flow diagram tracing how people move from workclass to broad education level to income bracket.")
pc_df = filtered.dropna(subset=["workclass"]).copy()
pc_df["edu_level"] = pc_df["education.num"].apply(
    lambda n: "Basic" if n <= 8 else ("HS/Some College" if n <= 12 else "Higher Ed")
)
fig12 = px.parallel_categories(
    pc_df, dimensions=["workclass", "edu_level", "income"],
    color=pc_df["income"].astype("category").cat.codes, color_continuous_scale=px.colors.sequential.Inferno
)
